...
public class MoveWithinWalls extends JFrame implements KeyListener {
	private JLabel[] myLabels;
	
	Icon actor=new ImageIcon(getClass().getResource("actor.jpg"));
	Icon iconEmpty=new ImageIcon(getClass().getResource("empty.jpg"));
	Icon iconBorder=new ImageIcon(getClass().getResource("border.jpg"));
	...
	public MoveWithinWalls()
	{
		...
		for(i=0;i<row;i++)
		{
			for(int j=0;j<column;j++)
			{
				if(!isAllowed(i,j))
				{
					myLabels[i*column+j]=new JLabel(iconBorder);
				}else{
					myLabels[i*column+j]=new JLabel(iconEmpty);
				}
				
				add(myLabels[i*column+j]);
			}
		}
